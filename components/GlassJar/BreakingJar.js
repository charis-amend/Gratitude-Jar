...
import { BackSide, MeshPhysicalMaterial, NormalBlending } from 'three';
import { useFrame } from "@react-three/fiber";
import { useEffect } from "react";
import { useGLTF } from "@react-three/drei";
import { useRef } from "react";

export default function BreakingJar({ clickState }) {
  ...
  useEffect(() => {
    // calculating position of each mesh (shattered piece of the jar):
    const groupWorldPosition = new THREE.Vector3();
    jarGroup.current.getWorldPosition(groupWorldPosition);

    jarGroup.current.children.forEach((mesh) => {
  ...
  useFrame(() => {
    jarGroup.current.children.forEach((mesh) => {
      // if (scrollData.offset < 0.00001) {
      if (!clickState) {
        if (mesh.name === "defaultMaterial") {
          mesh.visible = true
        } else {
          mesh.visible = false;
        }
      ...
      mesh.position.x = THREE.MathUtils.lerp(
        ...
        // scrollData.offset, // 0->start and 1-> after scroll
        clickState,
      )
      ...
      if (enableRotation) {
        mesh.rotation.x = THREE.MathUtils.lerp(
          ...
          clickState // 0 at the beginning and 1 after scroll
        );
